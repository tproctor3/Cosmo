...
class CozMap:
    ...
    def compute_smooth_path(self):
        ...
        start_ind = 0
        while start_ind < len(path) - 1:
            smooth_ind = len(path) - 1
            while start_ind < smooth_ind:
                if not self.is_collision_with_obstacles((path[start_ind], path[smooth_ind])):
                    smooth.append(path[smooth_ind])
                    start_ind = smooth_ind
                    break
                else:
                    smooth_ind -= 1
